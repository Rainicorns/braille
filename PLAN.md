...
**Phase 18 — Range API Expansion (Steps 0-3) COMPLETE.** Sparse checkout added `dom/ranges/` (39 test files). Replaced blanket `("Range"/"range")` skip with targeted skips for mutation tracking, cross-doc adoption, shadow DOM ranges, Selection API. Registered `Range` global constructor via `FunctionObjectBuilder` with `.constructor(true)`; `Range.prototype` cached in `RealmState::range_proto` field; constants `START_TO_START/END/etc` on both constructor and prototype. New methods: `collapsed` getter, `commonAncestorContainer` getter (ancestor chain walk), `detach()` (no-op per spec), `collapse(toStart)`, `cloneRange()`, `selectNode(node)`, `selectNodeContents(node)`, `toString()` (walks text nodes in document order), `compareBoundaryPoints(how, sourceRange)`, `comparePoint(node, offset)`, `isPointInRange(node, offset)`, `intersectsNode(node)`, `cloneContents()`. Core helper `compare_boundary_points_impl()` uses `DomTree::compare_document_position()` with correct DISCONNECTED/PRECEDING/CONTAINED_BY handling. `validate_boundary()` throws IndexSizeError for offset > nodeLength, InvalidNodeTypeError for Doctype. Cross-parent delete/extract refactored to use common ancestor walk. `range.rs` refactored from per-instance ObjectInitializer to shared Range.prototype via `create_range_prototype()`. Expected failures for cross-document xmlDoc/foreignDoc test cases. **229/314 WPT tests passing (0 fail, 85 skip). +18 new Range tests, +39 total test files.**
...
| Component | Gap |
|-----------|-----|
| WPT harness | **Phase 11 complete** (190/263 passing, 1 fail, 72 skip). Phase 11: Range API (createRange, deleteContents, extractContents, insertNode, surroundContents). Phase 10: shared Attr identity cache, DFS getElementById, NamedNodeMap caching, Attr.value sync, namespace validation, outerHTML setter. Phase 9: live NamedNodeMap via Proxy-based collection with Attr identity. Phase 8: location.hash, fragment activation, handleEvent TypeError, ErrorEvent dispatch. Earlier: MO unskip A-C, Phase 7 inline handlers + promise_test, Phase 6 activation + on* handlers + MouseEvent, Phase 5 MutationObserver + quick wins. Remaining ~72 skipped need Shadow DOM/workers/advanced iframes. |
