...
fn link_click_returns_navigate_action_with_correct_url() {
    ...
    assert!(snapshot.contains("link @e1"), "snapshot should contain link ref: {}", snapshot);
    assert!(snapshot.contains("Visit Example"), "snapshot should contain link text: {}", snapshot);
...
fn form_with_inputs_visible_in_snapshot() {
    ...
    assert!(snapshot.contains("input[type=text] @e1 value=\"alice\""),
        "snapshot should show username input with value: {}", snapshot);
    assert!(snapshot.contains("input[type=email] @e2 value=\"alice@example.com\""),
        "snapshot should show email input with value: {}", snapshot);
    assert!(snapshot.contains("button @e3 \"Submit\""),
        "snapshot should show submit button: {}", snapshot);
}
...
fn type_into_input_then_snapshot_shows_value() {
    ...
    let snapshot = engine.snapshot(SnapMode::Accessibility);
    assert!(snapshot.contains("value=\"hello\""),
        "snapshot after typing should contain value=\"hello\": {}", snapshot);
}
...
fn type_into_textarea_then_snapshot_shows_text() {
    ...
    assert!(snapshot.contains("world"),
        "snapshot after typing into textarea should contain typed text: {}", snapshot);
}
...
fn type_overwrites_previous_value_in_snapshot() {
    ...
    assert!(snap1.contains("value=\"old\""), "initial snapshot should show old value: {}", snap1);

    engine.handle_type("@e1", "new").unwrap();

    let snap2 = engine.snapshot(SnapMode::Accessibility);
    assert!(snap2.contains("value=\"new\""), "snapshot after type should show new value: {}", snap2);
...
fn select_option_then_snapshot_shows_selected_value() {
    ...
    let snapshot = engine.snapshot(SnapMode::Accessibility);
    assert!(snapshot.contains("value=\"Green\""),
        "snapshot after select should show selected option text 'Green': {}", snapshot);
}
...
fn select_changes_selected_option_in_snapshot() {
    ...
    engine.load_html(html);
    let snap1 = engine.snapshot(SnapMode::Accessibility);
    assert!(snap1.contains("value=\"Medium\""),
        "initial snapshot should show Medium selected: {}", snap1);

    engine.handle_select("@e1", "l").unwrap();

    let snap2 = engine.snapshot(SnapMode::Accessibility);
    assert!(snap2.contains("value=\"Large\""),
        "snapshot after select should show Large selected: {}", snap2);
}
...
fn inline_script_creates_nested_dom_structure() {
    ...
    assert!(snapshot.contains("navigation"), "snapshot should contain navigation role: {}", snapshot);
    assert!(snapshot.contains("link @e1 \"Home\""), "snapshot should contain Home link: {}", snapshot);
    assert!(snapshot.contains("link @e2 \"About\""), "snapshot should contain About link: {}", snapshot);
}
...
fn script_creates_form_with_inputs() {
    ...
    assert!(snapshot.contains("form"), "snapshot should contain form: {}", snapshot);
    assert!(snapshot.contains("input[type=text] @e1"), "snapshot should contain text input: {}", snapshot);
    assert!(snapshot.contains("button @e2 \"Go\""), "snapshot should contain Go button: {}", snapshot);
}
...
fn external_script_modifies_dom() {
    ...
    assert!(snapshot.contains("heading[1] \"Loaded from external\""),
        "external script should have created h1: {}", snapshot);
}
...
fn external_script_with_full_url() {
    ...
    assert!(snapshot.contains("Widget loaded"),
        "external script from full URL should execute: {}", snapshot);
}
...
fn mixed_scripts_execute_in_document_order() {
    ...
    assert!(snapshot.contains("ext-1,inline-2,ext-2"),
        "scripts should execute in document order: {}", snapshot);
}
...
fn external_script_sets_global_used_by_inline() {
    ...
    assert!(snapshot.contains("Config: production"),
        "inline script should see global set by external: {}", snapshot);
}
...
fn focus_input_shows_focused_marker_in_snapshot() {
    ...
    assert!(snapshot.contains("input[type=text] @e1") && snapshot.contains("[focused]"),
        "the input should be focused: {}", snapshot);
}
...
fn focus_moves_between_elements() {
    ...
    engine.handle_focus("@e1").unwrap();
    let snap1 = engine.snapshot(SnapMode::Accessibility);
    assert!(snap1.contains("@e1") && snap1.contains("[focused]"),
        "first input should be focused: {}", snap1);
    ...
    assert!(!e1_line.contains("[focused]"), "first input should not be focused anymore: {}", snap2);
...
fn blur_clears_focused_marker() {
    ...
    engine.handle_blur();
    let snap2 = engine.snapshot(SnapMode::Accessibility);
    assert!(!snap2.contains("[focused]"), "focus should be cleared after blur: {}", snap2);
}
...
fn refs_reset_after_loading_new_html() {
    ...
    assert!(snapshot.contains("button @e1"), "new @e1 should be button: {}", snapshot);
...
fn all_interactive_elements_get_unique_sequential_refs() {
    ...
    assert!(snapshot.contains("link @e1 \"Link 1\""),
        "link should be @e1: {}", snapshot);
    assert!(snapshot.contains("button @e2 \"Button 1\""),
        "button should be @e2: {}", snapshot);
    assert!(snapshot.contains("input[type=text] @e3"),
        "text input should be @e3: {}", snapshot);
    assert!(snapshot.contains("select @e4"),
        "select should be @e4: {}", snapshot);
    assert!(snapshot.contains("textarea @e5"),
        "textarea should be @e5: {}", snapshot);
    assert!(snapshot.contains("link @e6 \"Link 2\""),
        "second link should be @e6: {}", snapshot);
    assert!(snapshot.contains("button @e7 \"Button 2\""),
        "second button should be @e7: {}", snapshot);
    assert!(snapshot.contains("input[type=email] @e8"),
        "email input should be @e8: {}", snapshot);

    // Verify all refs resolve
    for i in 1..=8 {
        let ref_str = format!("@e{}", i);
        assert!(engine.resolve_ref(&ref_str).is_some(),
            "{} should resolve to a node", ref_str);
    }
...
fn script_builds_complex_nested_structure_with_attributes() {
    ...
    assert!(snapshot.contains("link @e1 \"Home\""), "should have Home link: {}", snapshot);
    assert!(snapshot.contains("link @e2 \"About\""), "should have About link: {}", snapshot);
    assert!(snapshot.contains("main"), "should have main: {}", snapshot);
    assert!(snapshot.contains("heading[1] \"Dashboard\""), "should have h1: {}", snapshot);
    assert!(snapshot.contains("paragraph \"Welcome to the dashboard.\""),
        "should have paragraph: {}", snapshot);
    assert!(snapshot.contains("form"), "should have form: {}", snapshot);
    assert!(snapshot.contains("input[type=text] @e3 value=\"search here\""),
        "should have search input with value: {}", snapshot);
    assert!(snapshot.contains("button @e4 \"Search\""), "should have search button: {}", snapshot);
...
fn script_modifies_text_content_of_existing_elements() {
    ...
    assert!(snapshot.contains("Modified Title"),
        "title should be modified: {}", snapshot);
    assert!(snapshot.contains("Modified description"),
        "description should be modified: {}", snapshot);
    assert!(!snapshot.contains("Original Title"),
        "original title should be gone: {}", snapshot);
    assert!(!snapshot.contains("Original description"),
        "original description should be gone: {}", snapshot);
}
...
fn click_on_nonexistent_ref_returns_error() {
    ...
    match action {
        EngineAction::Error(msg) => {
            assert!(msg.contains("element not found") || msg.contains("not found"),
                "error message should indicate element not found: {}", msg);
        }
...
fn full_workflow_load_type_focus_snapshot() {
    ...
    assert!(snap1.contains("input[type=text] @e3"), "search input should be @e3: {}", snap1);
    assert!(snap1.contains("button @e4 \"Search\""), "search button should be @e4: {}", snap1);
    ...
    let snap2 = engine.snapshot(SnapMode::Accessibility);
    assert!(snap2.contains("value=\"braille browser\""),
        "input should have the typed value: {}", snap2);
    assert!(snap2.contains("button @e4 \"Search\" [focused]"),
        "search button should be focused: {}", snap2);
}
...
fn type_and_select_then_verify_combined_state() {
    ...
    let snapshot = engine.snapshot(SnapMode::Accessibility);
    assert!(snapshot.contains("value=\"Alice\""),
        "name input should have Alice: {}", snapshot);
    assert!(snapshot.contains("value=\"Canada\""),
        "select should show Canada: {}", snapshot);
    assert!(snapshot.contains("Software engineer"),
        "textarea should show bio: {}", snapshot);
}
...
fn multiple_load_cycles_reset_state_cleanly() {
    ...
    assert!(snap1.contains("value=\"typed on page 1\""), "should see typed text: {}", snap1);
    ...
    assert!(!snap2.contains("Page 1 Link"), "page 1 content should be gone: {}", snap2);
    assert!(!snap2.contains("typed on page 1"), "typed text should be gone: {}", snap2);
    assert!(!snap2.contains("[focused]"), "focus should be reset: {}", snap2);

    // Page 2 content should be present
    assert!(snap2.contains("heading[1] \"Page 2\""), "should see page 2 heading: {}", snap2);
    assert!(snap2.contains("button @e1 \"Action\""), "should see page 2 button as @e1: {}", snap2);
}
...
fn external_script_missing_from_fetched_is_skipped_gracefully() {
    ...
    assert!(snapshot.contains("Still works"),
        "inline script should execute even when external is missing: {}", snapshot);
}
...
fn script_with_src_attribute_ignores_inline_content() {
    ...
    assert!(snapshot.contains("GOOD EXTERNAL"),
        "external content should run: {}", snapshot);
    assert!(!snapshot.contains("BAD INLINE"),
        "inline content should be ignored when src is present: {}", snapshot);
}
...
fn resolve_ref_before_snapshot_returns_none() {
    ...
    assert_eq!(engine.resolve_ref("@e1"), None,
...
fn snapshot_with_deeply_nested_interactive_elements() {
    ...
    assert!(snapshot.contains("link @e1 \"Deep Link\""),
        "deeply nested link should have ref: {}", snapshot);
    assert!(snapshot.contains("button @e2 \"Deep Button\""),
        "deeply nested button should have ref: {}", snapshot);
