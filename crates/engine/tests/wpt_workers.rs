//! WPT (Web Platform Tests) runner for workers/.
//!
...
fn should_skip(rel_path: &str) -> Option<&'static str> {
    let skip_patterns: &[(&str, &str)] = &[
        ("SharedWorker", "requires SharedWorker support"),
        (".worker.js", "requires actual worker thread"),
...
fn main_inner() {
    ...
    let wpt = wpt_root();
    let workers_root = wpt.join("workers");

    let mut trials = Vec::new();

    let test_files = discover_tests(&workers_root);
